...
public class DecompressorExecuter extends Executer {
	...
	protected static void process() throws IOException {
		...
		for (int idsIdx = 0; idsIdx < idsList.length; idsIdx++) {
			...
			String log = "Decompressed " + idsIdx + " " + nameList[idsIdx] + " time " + decompressor.time() / 1000
					+ " sec";
			logs.add(log);
			...
			try {
				...
				Files.write(Paths.get(parameters.getOutdir(), "\\decompresstiming.txt"), timing);
				Files.write(Paths.get(parameters.getOutdir(), "\\decompresslog.txt"), logs);
			} catch (IOException e) {
