...
public class CUCompressor extends Compressor {
	...
	public double[][] process() {
		...
		Pointer reverseVecKernelParams = Pointer.to(Pointer.to(new int[] { nSamples }), Pointer.to(deviceData),
		...
		for (int fIdx = 0; fIdx < nParts; fIdx++) {
			...
			JCudaDriver.cuLaunchKernel(memSetKernel, gridSizeX, 1, 1, blockSizeX, 1, 1, 0, null, memSetKernelParams,
					null);
			cuCtxSynchronize();

			// coefflimit kernel
			Pointer limitCoeffKernelParam = Pointer.to(Pointer.to(new int[] { nBatch }), Pointer.to(new int[] { rbs }),
					Pointer.to(new float[] { parameters.getCoeffLimit() }), Pointer.to(dDArrays), Pointer.to(dRArrays),
					Pointer.to(dCArrays));

			// sumSquareError kernel
			Pointer sumSquareErrorKernelParams = Pointer.to(Pointer.to(new int[] { nBatch }),
					Pointer.to(new int[] { rbs }), Pointer.to(new int[] { nCoeff }),
					Pointer.to(new float[] { parameters.getCoeffLimit() }), Pointer.to(dDArrays), Pointer.to(dCArrays),
					Pointer.to(dEArrays), Pointer.to(dSSEArrays));
